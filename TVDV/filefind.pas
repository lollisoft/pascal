...
Begin
  ...
  BEGIN
    ...
    if (Filesize(ZusatzDataFile) < Longint(DateiData.Loc.Ptr) - 1) or

       (Longint(DateiData.Loc.Ptr) < 1) then

    Begin

      Str(FileSize(ZusatzDataFile), Z);

      Str(Longint(DateiData.Loc.Ptr) - 1, Z1);
    ...
    seek(ZusatzDataFile, Longint(DateiData.Loc.Ptr)-2+A);

    Read(ZusatzDataFile, ZusatzData);



    If (Longint(ZusatzData.Sl.Ptr) > Filesize(StrucktDataFile)) or

       (Longint(ZusatzData.Sl.Ptr) < 1) then

    Begin

      Str(Longint(ZusatzData.Sl.Ptr) - 1, Z);
    ...
    Seek(StrucktDataFile, Longint(ZusatzData.Sl.Ptr)-1);
    ...
    If (Longint(ZusatzData.Sp.Ptr) > Filesize(StrucktDataFile)) or

       (Longint(ZusatzData.Sp.Ptr) < 1) then

    Begin

      Str(Longint(ZusatzData.Sp.Ptr) - 1, Z);
    ...
    Seek(StrucktDataFile, Longint(ZusatzData.Sp.Ptr)-1);
...
BEGIN
  ...
  MsgRes := DeskTop^.ExecView (Application^.ValidView (Dialog));
...
Begin
  ...
  MsgRes := Message(TopView, evCommand, cmSelectLabel, @Item)
...
Begin
  ...
  Begin
    ...
    MsgRes := Message(Owner, evBroadCast, cmLabelFocused, @Self)
...
Begin
  ...
  Begin
    ...
    If Mehrfache = False then
      ...
      If PData^.PasstinMaske(ListMaske, DateiData.DateiName) then
    ...
    Else
      ...
      If PData^.PasstinMaske(ListMaske, DateiData.DateiName) And
