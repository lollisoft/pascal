...
{ AltF1 - Taste: }
  ...
  KeyRefListPtrtyp = ^KeyRefListtyp;
  KeyRefListtyp    = Record
                       KeyRef    : Integer;
                       Next,
                       Back      : KeyRefListPtrtyp
                     End;

Const KeyRefList: KeyRefListPtrtyp = Nil;
...
var
  ...
  Para: PParagraph;
const
  InvalidStr = #13' Keine Hilfe fr diesen Context vorhanden.';
...
begin
  ...
  If KeyRefList = Nil then
  Begin
    New(KeyRefList);
    KeyRefList^.Back := Nil;
    KeyRefList^.Next := Nil;
    KeyRefList^.KeyRef := Context;
  (*KeyRefList^.KeyPoint := KeyPoint;
    KeyRefList^.KeyLength := KeyLength;
    KeyRefList^.KeyCount := Selected;
  *)
  End
  Else
  Begin
    ...
    KeyRefList^.Next^.Back := KeyRefList;

    KeyRefList := KeyRefList^.Next;
    ...
    evKeyDown:
      Begin
        case Event.KeyCode of
          ...
          kbAltF1:
            Begin
              If (KeyRefList <> Nil) and
                 (KeyRefList^.Back <> Nil) then
              Begin
                KeyRefList := KeyRefList^.Back;

                KeyRef := KeyRefList^.KeyRef;
                Dispose(KeyRefList^.Next);
                KeyRefList^.Next := Nil;
...
begin
  R.Assign(0, 0, 80, 25);
  TWindow.Init(R, 'Hilfe', wnNoNumber);
