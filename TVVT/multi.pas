...
Const    (*  text  : String = ''; *)       (* Ausgabezeile         *)
      tmp   : Integer = 0;            (* Zeichenzahl          *)
...
Procedure Wait(txt : string ; w : integer);
Procedure CursorAus;
...
(*Procedure getword(Name, Muster: String);*)
...
Procedure getword;
{---------------------------------------------------------}
{      zÑhle die Zahl der Worte in der Zeile              }
{---------------------------------------------------------}
Var i : Integer;
begin
 FOR i := 0 TO lang DO                {* scan line             *}
  begin
   IF UpCase(CHR(Buffer[i])) IN ['A'..'Z','é','ô','ö',
        'Ñ','Å','î','·',' ',',','-',':','.'] THEN
    begin
     tmp := tmp + 1;                  {* zÑhle Buchstaben      *}
     text := text + Chr(Buffer[i]);   {* merke Zeichen         *}
    end
   ELSE
    begin
     IF Pos(UpDate(Muster), UpDate(Text)) <> 0 THEN
     Begin
       writeln (TextFile, Name);                {* gebe Satz aus         *}
       Flush(TextFile)
     End;
     tmp := 0;                        {* clear char count      *}
     text := '';                      {* clear Text            *}
    end;
  end; {* For *}
end; { getword }
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      If Root^.Anzahl < 0 then
      Begin
        Writeln('Fehler in Anzahl eines Elementes!');
        Halt
      End;
      Inc(Anzahl);
...
{  Anzahl := 0;
  ...
  Begin
    Wait('Kein Speicher fÅr diese Operation mehr da!',0);
    Halt
  End;
  ...
  BEGIN
    ...
    BEGIN
      ...
      If Not (HilfListEnd^.HilfData.Art in [3,4]) then
        Wait('Beim Laden der Hilf - Datei wurde ein Fehler gefunden ( Art <> 3, 4 )!',0);
...
(* ---    Umwandeln des Restes    --- *)
  ...
  GlobalFeldPtr := Pointer(PfadFeld);
  MachBinBaum(GlobalListPtrtyp(PfadList),1,PAnzahl);
  ReleaseArray(PfadFeld,4,PAnzahl, EMSPF);
  PfadFeld := Nil;

  GlobalFeldPtr := Pointer(LabelFeld);
  MachBinBaum(GlobalListPtrtyp(LabelList),1,LAnzahl);
  ReleaseArray(LabelFeld,4,LAnzahl, EMSLF);
  LabelFeld := Nil;
...
Procedure Wait;
begin
  MessageBox(txt, nil, mfError + mfOkButton)
end;
...
begin
  ...
  begin
    ...
    begin
      SpeichereDatei(DateiName, lauf^.links,dif);
      ...
      if DateiName > Lauf^.DateiData.DateiName then
      begin
        SpeichereDatei(DateiName, lauf^.rechts,dif);
...
begin
  ...
  if LabelName < Lauf^.LabelData.LabelName then
{ begin }
    SpeichereLabel(LabelName,lauf^.links,dif)
{
    if dif then
    case lauf^.bal of
  ...
  if LabelName > Lauf^.LabelData.LabelName then
{ begin }
    SpeichereLabel(LabelName, lauf^.rechts,dif)
{
    if dif then
    case lauf^.bal of
    -1 : begin lauf^.bal := 0; dif := false end;
...
begin
  ...
  if PfadName < Lauf^.PfadData.PfadName then
{ begin }
    SpeicherePfad(PfadName,lauf^.links,dif)
{
    if dif then
    case lauf^.bal of
  ...
  if PfadName > Lauf^.PfadData.PfadName then
{ begin }
    SpeicherePfad(PfadName,lauf^.rechts,dif)
{
    if dif then
    case lauf^.bal of
    -1 : begin lauf^.bal := 0; dif := false end;
...
begin
  ...
  begin
    ...
    if dp^.DateiData.Loc^.Next = Nil then ZusatzListend := dp^.DateiData.Loc
                                     else
                                     begin
...
begin
  ...
  begin
    writeln;
    write('Label-Datei kann nicht geîffnet werden !');
    delay(1000);
    halt
  end
  (*$I+*)
